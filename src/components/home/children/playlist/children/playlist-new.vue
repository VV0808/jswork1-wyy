  ...
  <section class="playlist-new">
    <div class="channel">
      ...
      <div class="hide-on-large-only">
        <ul class="list">
        <router-link tag="li" :to="'/songListDetail?id='+item.id" class="item" v-for="(item,index) in playList"
                     :key="index" style="width:45%">
          <div class="icon">
            <img class="avatar" v-lazy="item.coverImgUrl">
            <div class="playCount"><span class="icon-play"></span> {{item.playCount | numToWan}}</div>
          </div>
          <div class="name" style="font-size:12px;line-height:13px">{{item.name}}</div>
          <div class="creator" style="font-size:13px;line-height:14px">{{item.creator.nickname}}</div>
        </router-link>
      </ul>
      <button :disabled="loading" class="loadMore-text" @click="loadMore" style="font-size:13px;">加载更多</button>
      </div>
    </div>
  </section>
</template>
