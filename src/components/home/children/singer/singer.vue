<template>
  <section class="singer">
    <div class="hide-on-med-and-down">
      ...
      <router-view class="singer-d"></router-view>
    </div>
    <div class="hide-on-large-only">
      <ul class="list">
        <li class="item" v-for="item in artistList" @click="selectItem(item)" style="width:30%">
          <div class="avatar">
            <img v-lazy="item.img1v1Url">
          </div>
          <div class="name" style="font-size:12px">{{item.name}}</div>
        </li>
        <button :disable="disable" class="loadMore-text" @click="loadMore" v-show="hasMore">加载更多</button>
      </ul>
